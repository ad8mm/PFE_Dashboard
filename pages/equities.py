...
def fetch_yahoo_equity_ohlc(ticker, range_="1y", interval="1d"):
...
st.title("📈 Suivi des Actions Cotées")

# Liste de base
default_tickers = ["AAPL", "MSFT", "GOOGL"]

# Barre de recherche
user_input = st.text_input("🔍 Ajouter une action (TOP 10 US)", placeholder="ex: Apple, Tesla...").lower().strip()
...
selected_ticker = st.selectbox("📌 Sélectionnez une action :", default_tickers)
...
df_chart = fetch_yahoo_equity_ohlc(selected_ticker, range_=period, interval=interval)
